def download_merged_names(name_without_ext):
    import pandas as pd
    ...
    try:
        input_path = os.path.join('seperated', f'{name_without_ext}_predicted_1.csv')
        output_path = os.path.join('cleaned', f'{name_without_ext}_cleaned_predicted_1.csv')
        ...
        for record_id, group in df.groupby('record_id'):
            ...
            contributing_urls = sorted(
                contributing_urls,
                key=lambda url: priority_map.get(extract_domain(url), default_priority)
            )
        ...
        return {
            "status": "error",
            "error_message": f'cannot find the column - {str(e)}',
